...
def test_parse_yaml_file():
    ...
    assert set(config.exclude_dirs) == set(['tests', '**/temp*','.git', '.cache'])
    assert set(config.exclude_files) == set(['example.txt'])
...
def test_parse_toml_file():
    ...
    assert set(config.exclude_dirs) == {'tests', '**/temp*'}
    assert set(config.exclude_files) == {'example.txt'}
    assert config.language == 'fr'
    assert config.additional_chars == 'ü,ö,ß'
    assert config.verbose == True
